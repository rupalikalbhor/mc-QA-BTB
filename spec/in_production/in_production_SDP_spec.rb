...
describe 'SDP - In Production' do
  ...
  context 'I. Breadcrumb' do
    ...
    it '3. Verify breadcrumb displays in this format ModCloth » Be The Buyer » Voting In Progress » <Selected sample number>' , :no_phone => true do
      page.find(:xpath, "//div[@id='breadcrumbs']", :visible => true).text.should == 'ModCloth » Be The Buyer » In Production » '+$sample_name
    end
  end

  context 'II. Left side section', :no_phone => true do
    it '1. Verify "In Production" section with correct text is displayed.' do
      within ('.state-instructions-sharing-section') do
        page.should have_content ('In Production')
        page.should have_content ("This winning item is in the works! Sign up to be emailed the moment it makes the big debut.")
      end
    end

    it '2. Verify Available Soon text is displaying.' do
      page.find(:xpath, "//div[@class = 'availability']", :visible => true).text.should == "Available Soon"
    end
  ...
  context 'VI. Arrows' do
    ...
    it "2. Verify when user is on last sample SDP page then user see only 'Prev' arrow." do
      go_to_in_production_page
      sample_count = get_in_production_SampleCount
