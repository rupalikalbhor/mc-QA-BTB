...
describe 'Grid page - Awaiting Results' do
  ...
  context "I. UI" do
    ...
    it '2. Verify page title is correct', :no_phone => true do
      page.find(:xpath, "//div[@class='sample-grid']/nav/h2[@class='page-title awaiting-results']", :visible => true).text.should == page_title
    end
    ...
    it '6. Verify breadcrumb displays correct sequence with text', :no_phone => true do
      page.find(:xpath, "//div[@id='breadcrumbs']", :visible => true).text.should == 'ModCloth » Be The Buyer » Awaiting Results'
    end
  ...
  context 'II. Sample Box' do
    ...
    it '7. Verify sample displays "Voting End date" with clock icon.' do
      ...
      page.find(:xpath, "//div[@data-product-id="+FIRST_SAMPLE_PRODUCT_ID+"]/div[@class = 'awaiting-results status']", :visible => true).text.should == "Ended "+voting_ends_at_from_db
    end
